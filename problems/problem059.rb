...
time = Benchmark.realtime do
  File.open(File.expand_path(File.dirname(__FILE__)) + "/../inputs/problem059.txt", "r") do |infile|
    chars = infile.readline.split(',').to_a.map(&:to_i)
    letter_permutations = ("a".."z").to_a.permutation(3)
    letter_permutations.each do |perm|
      str = String.new
      perm_ord = perm.map(&:ord)
      chars.each_with_index do |value, key|
        str += (value^perm_ord[key%3].to_i).chr
      end

      if str.match /^[0-9a-zA-Z\s\.,\(\)!;\-']*$/
        @answer = str.chars.to_a.map(&:ord).inject(:+)
        break
      end
    end
  end
end
puts "Euler 59: #{@answer}\t#{time*1000} milliseconds"